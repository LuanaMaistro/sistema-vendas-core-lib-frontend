...
export default class AddStockUseCase extends UseCase {
  ...
  async execute(command: AddStockCommand): Promise<OperationResult<Product>> {
    ...
    return this.toOperationResult(
      await this.productService.AddStock(command.id, Quantity.create(command.quantity))
    )
  }
}
